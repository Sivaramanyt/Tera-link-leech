...
async def stream_upload_media(context, chat_id: int, file_path: str, filename: str) -> bool:
    ...
    logger.info(f"🚀 Starting streaming upload: {filename} ({file_size/(1024*1024):.1f} MB)")
    logger.info(f"🧠 Pre-upload memory: {available_mb:.1f}MB available")

    if available_mb < 150:
        raise Exception(f"❌ Insufficient memory for upload: {available_mb:.1f} MB available")
